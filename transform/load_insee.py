...
# Charger INSEE pour toutes les années
fichiers = config["insee"]["fichiers"]
years = list(fichiers.keys())

print("--- Chargement INSEE dans staging ---")
for annee in years:
    path = os.path.join(BASE_DIR, f"data/bronze/insee/annee={annee}/insee_{annee}.parquet")
    df = pd.read_parquet(path)
    load_to_staging(df, f"insee_{annee}")

print("\n✅ INSEE staging terminé !")